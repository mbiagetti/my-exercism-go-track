...
// Use should call Close but not Defrob on non-Error panic from Frob
// and return a custom the error.
func TestCallCloseOnNonFrobErrorWithPanicWithoutError(t *testing.T) {
	var closeCalled bool
	var defrobCalled bool
	mr := mockResource{
		close:  func() error { closeCalled = true; return nil },
		frob:   func(input string) { panic("meh") },
		defrob: func(tag string) { defrobCalled = true },
	}
	opener := func() (Resource, error) { return mr, nil }
	err := Use(opener, hello)
	...
	if err.Error() != "panic without error" {
		t.Fatalf("Invalid error returned from Use")
	}
	if defrobCalled {
		t.Fatalf("Defrob was called")
